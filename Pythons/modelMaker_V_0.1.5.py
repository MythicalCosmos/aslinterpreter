...
class GestureApp:
    ...
    def toggle_capture(self):
        selected_paths = [p for p, var in self.checkbox_vars.items() if var.get()]
        if not selected_paths:
            messagebox.showwarning("No Selection", "Select at least one gesture using the checkboxes to capture images.")
            return
    ...
    def refresh_gesture_list(self):
        ...
        # Note: Treeview checkboxes are simulated in this version by mapping BooleanVars to each path
    ...
    def delete_gesture(self):
        ...
        if not to_delete:
            messagebox.showwarning("Select Gesture", "Select gestures to delete using the checkboxes.")
            return
    ...
    def rename_gesture(self):
        ...
        if len(to_rename) != 1:
            messagebox.showwarning("Select One", "Select exactly one gesture to rename using the checkbox.")
            return
        path = to_rename[0]
    ...
    def delete_image(self):
        ...
        if len(to_select) != 1:
            messagebox.showwarning("Select One", "Select exactly one gesture to delete images from using the checkbox.")
            return
        path = to_select[0]
    ...
    def train_selected(self):
        selected_paths = [p for p, var in self.checkbox_vars.items() if var.get()]
        if not selected_paths:
            messagebox.showwarning("No Selection", "Select gestures using the checkboxes to train.")
            return
