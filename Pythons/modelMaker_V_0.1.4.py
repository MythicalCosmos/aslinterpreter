...
class GestureApp:
    ...
    def build_gui(self):
        ...
        ttk.Label(control_frame, text="Hand:").grid(row=0, column=0, padx=5)
        self.combo_hand = ttk.Combobox(control_frame, textvariable=self.current_hand, values=["left", "right"], width=10)
        self.combo_hand.grid(row=0, column=1, padx=5)
        self.combo_hand.current(0)

        ttk.Button(control_frame, text="📸 Capture", command=self.toggle_capture).grid(row=0, column=2, padx=5)
        ttk.Button(control_frame, text="Add Gesture", command=self.add_gesture).grid(row=0, column=3, padx=5)

        table_frame = ttk.LabelFrame(self.root, text="Gestures")
        table_frame.pack(fill="both", expand=True, padx=10, pady=10)

        self.tree = ttk.Treeview(table_frame, columns=("hand", "count", "path"), show="headings", selectmode="extended")
    ...
    def toggle_capture(self):
        selection = self.tree.selection()
        if not selection:
            messagebox.showwarning("No Selection", "Select at least one gesture in the table to capture images.")
            return
    ...
    def train_selected(self):
        selection = self.tree.selection()
        if not selection:
            messagebox.showwarning("No Selection", "Select gestures in the table to train/export.")
            return
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            try:
                ...
                model.export_model()
                messagebox.showinfo("Training & Export Complete", f"Accuracy: {acc:.2f}\nLoss: {loss:.2f}\nModel exported to ./exported_model_gui")
