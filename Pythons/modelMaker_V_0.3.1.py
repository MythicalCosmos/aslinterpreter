...
class GestureApp:
    ...
    def toggle_capture(self):
        ...
        print(f"Capture {state} for selected gestures.")
    ...
    def train_selected(self):
        ...
        # Filter only non-empty gesture folders
        valid_gestures = {}
        for gesture_dir in selected_paths:
            ...
            if images:
                ...
                print(f"Skipping empty gesture folder: {gesture_dir}")

        if not valid_gestures:
            print("No valid gestures with images selected.")
            return
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            # Wrap MediaPipe training in try/except to suppress GUI error popups
            try:
                ...
                if not data.labels:
                    print("No labels found after dataset creation. Training aborted.")
                    return
                ...
                print(f"Training complete! Accuracy: {acc:.2f}, Loss: {loss:.2f}, Model exported to {export_path}")
    ...
    def export_gestures(self):
        if not self.trained or not self.trained_model_path:
            print("Model not trained yet. Cannot export.")
            return
        ...
        try:
            ...
            print(f"Model exported successfully to {os.path.join(dest, 'gesture_model')}")
        except Exception as e:
