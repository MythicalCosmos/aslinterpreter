...
class GestureApp:
    ...
    def train_selected(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            # Only create left/right folders if needed
            hands = ["left", "right"]
            for hand in hands:
                os.makedirs(os.path.join(tempdir, hand), exist_ok=True)

            # Copy only non-empty gesture folders
            copied = False
            for gesture_dir in selected_paths:
                ...
                target_dir = os.path.join(tempdir, hand, gesture_name)
                os.makedirs(target_dir, exist_ok=True)
                for img in images:
                    shutil.copy(os.path.join(gesture_dir, img), os.path.join(target_dir, img))
                copied = True
