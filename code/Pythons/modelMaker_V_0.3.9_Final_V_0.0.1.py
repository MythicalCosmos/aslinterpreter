...
from tkinter import ttk, messagebox, simpledialog, Label,  Entry, StringVar, Frame, Scale, Listbox, HORIZONTAL, filedialog, messagebox, ttk, OptionMenu, IntVar
...
class GestureApp:
    ...
    def build_gui(self):
        ...
        self.root.rowconfigure(1, weight=1)  # main middle 
        self.root.rowconfigure(2, weight=1)  # bottom log 
        ...
        ttk.Button(control_frame, text="Train Selected", command=self.train_selected).grid(row=0, column=2, padx=5)
        ttk.Button(control_frame, text="Export Model", command=self.export_selected).grid(row=0, column=3, padx=5)
    ...
    def init_camera(self):
        ...
        print(f"Found Camera, Camera Opened? {self.cap.isOpened()}") 
    ...
    def train_selected(self):
        print("train button works")       
    def export_selected(self):
        print("export button works")
        self.log_status("Exporting Model")
        #REST OF CODE
        self.log_status("Model Exported Successfully")
