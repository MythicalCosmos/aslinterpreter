...
class GestureApp:
    ...
    def train_selected(self):
        ...
        for gesture_dir in selected_paths:
            images = [f for f in os.listdir(gesture_dir)
                      if f.lower().endswith((".jpg", ".jpeg", ".png")) and os.path.isfile(os.path.join(gesture_dir, f))]
            if images:
                valid_gestures[gesture_dir] = images
            else:
                print(f"Skipping '{gesture_dir}' – no valid images found")

        if not valid_gestures:
            messagebox.showerror("Training Error", "No gestures with valid images selected.")
            return

        with tempfile.TemporaryDirectory() as tempdir:
            for gesture_dir, images in valid_gestures.items():
                ...
                print(f"Including '{gesture_name}' with {len(images)} images")

            try:
                print("Creating MediaPipe Dataset...")
                data = gesture_recognizer.Dataset.from_folder(
                    dirname=tempdir,
                    hparams=gesture_recognizer.HandDataPreprocessingParams()
                )
                print(f"Dataset labels detected: {data.labels}")
                if not data.labels:
                    raise RuntimeError("No labels found. Check that gesture folders contain images.")

                train_data, rest_data = data.split(0.8)
                validation_data, test_data = rest_data.split(0.5)
                export_path = os.path.join(os.getcwd(), "exported_model_gui")
                ...
                self.trained = True
                self.trained_model_path = export_path
                messagebox.showinfo(
                    "Training Complete",
                    f"Trained gestures: {', '.join([os.path.basename(p) for p in valid_gestures.keys()])}\n"
                    f"Accuracy: {acc:.2f}, Loss: {loss:.2f}\nExported to {export_path}"
                )
            except Exception as e:
                traceback_str = traceback.format_exc()
                print("Training exception:\n", traceback_str)
                messagebox.showerror("Training Error", f"{str(e)}\nSee console for full traceback.")
                self.trained = False
                self.trained_model_path = None
    ...
    def export_gestures(self):
        if not self.trained or not self.trained_model_path:
            messagebox.showwarning("Not Trained", "Train gestures first.")
            return
        ...
        try:
            ...
            messagebox.showinfo("Export Complete", f"Model exported to {os.path.join(dest, 'gesture_model')}")
        except Exception as e:
