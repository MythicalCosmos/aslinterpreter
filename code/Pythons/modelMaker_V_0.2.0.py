    ...
    def train_selected(self):
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            for gesture_dir in selected_paths:
                ...
                if not images:
                    continue  # skip empty
                hand = os.path.basename(os.path.dirname(gesture_dir))
                gesture_name = os.path.basename(gesture_dir)
                if hand not in hand_to_gestures:
                    hand_to_gestures[hand] = []
                hand_to_gestures[hand].append((gesture_name, gesture_dir))

            if not hand_to_gestures:
                messagebox.showerror("Training Error", "No valid gestures with images selected.")
                return

            for hand, gestures in hand_to_gestures.items():
                hand_dir = os.path.join(tempdir, hand)
                os.makedirs(hand_dir, exist_ok=True)
                for gesture_name, src_dir in gestures:
                    dst_dir = os.path.join(hand_dir, gesture_name)
            ...
            try:
                ...
                if not data:
                    raise RuntimeError("Dataset is empty. Check gestures have images and proper folder structure.")
